...
class LatentBridge(nn.Module):
    def __init__(self, embed_dim=1024, target_channels=4):
        ...
        self.upconv = nn.ConvTranspose2d(in_channels=embed_dim, out_channels=256, kernel_size=2, stride=2)
        self.act = nn.GELU()
    ...
    def forward(self, vjepa_tokens, num_frames=16, height=256, width=256):
        T_p, H_p, W_p = num_frames // 2, height // 16, width // 16
...
def get_action(prev_state, curr_state):
    prev_xyz, prev_rvec_deg = prev_state[:3], prev_state[3:]
    curr_xyz, curr_rvec_deg = curr_state[:3], curr_state[3:]

    prev_rotation = Rotation.from_rotvec(np.deg2rad(prev_rvec_deg))
    curr_rotation = Rotation.from_rotvec(np.deg2rad(curr_rvec_deg))

    prev_rot_matrix_inv = prev_rotation.as_matrix().T
    delta_xyz = prev_rot_matrix_inv @ (curr_xyz - prev_xyz)

    delta_rotation_matrix = prev_rot_matrix_inv @ curr_rotation.as_matrix()
    delta_rvec_deg = np.rad2deg(Rotation.from_matrix(delta_rotation_matrix).as_rotvec())

    return np.concatenate([delta_xyz, delta_rvec_deg], dtype=np.float32)
...
def main():
    ...
    states = np.load(os.path.join(episode_path, "states_6dof.npy")).astype(np.float32)
    ...
    output_path = "rollout_recording.mp4"
    fps = 15.0
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    video_writer = cv2.VideoWriter(output_path, fourcc, fps, (crop_size * 3, crop_size))
    ...
    with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        for i in tqdm(range(us_file_count), desc="Recording Rollout Video"):
            ...
            combined_frame = np.hstack([gt_us_img, rgb_img, pred_img])[..., ::-1]
